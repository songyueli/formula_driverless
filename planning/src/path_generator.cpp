...
// REAL detected cone. Confirmed directly as a real, reproducible collision
// cause (2026-08-23 ground-truth accuracy session, see
// tools/eval/eval_localization.cpp): the car repeatedly stalled at the
// exact same world coords, right next to a real cone, in a section of
// track whose LOCAL geometry (confirmed via Gazebo's own scene service) is
// a straight, evenly-spaced run -- not a sharp corner -- pointing at a
// waypoint-placement bug rather than a genuinely hard turn.
// kAssumedHalfTrackWidth's own fixed-offset assumption is exactly the kind
// of thing that can silently aim a waypoint too close to (or past) a real
// cone whenever the actual local geometry doesn't match it, and the
// pairing algorithm has no equivalent safeguard (it only checks that a
// LEFT and RIGHT cone aren't implausibly far apart, never that the
// resulting MIDPOINT is far enough from either one, or from any OTHER
// nearby cone).
//
...
//
// Deliberately ONLY this clamp, not the companion SLAM-landmark-
// augmentation change tried alongside it in the same session (see
// planning.cpp's git history / feedback_no_reverse_recovery's sibling
// project memory) -- that companion change introduced a NEW near-spawn
// stall (pose/yaw not yet converged at startup feeding a bad body-frame-
// transformed landmark into the very first few cycles) and was reverted.
// This clamp alone is a pure, self-contained function of already-computed
// waypoints and already-visible cones -- no new subscriptions, no new
// state, nothing that depends on how fresh/converged localization is yet.
...
std::vector<PathPoint> SingleSideOffsetPath(const std::vector<ClassifiedCone> &_side, double _sign)
{
    ...
    for (const auto &c : _side)
    {
        ...
        // here could possibly be too close to.
